    ...
    public class SeedController : ControllerBase
    {
        ...
        public async Task<ActionResult> Import()
        {
            ...
            var path = Path.Combine(
                ...
                "Data/Source/BlockingBikesDataSet.xlsx");
            ...
            var numberOfAddressesAdded = 0;
            var numberOfVehiclesAdded = 0;

            // create a lookup dictionary
            // containing all the countries already existing
            // into the Database (it will be empty on first run).
            var addressesByName = _context.Addresses
                .AsNoTracking()
                .ToDictionary(x => x.Location, StringComparer.OrdinalIgnoreCase);
            // iterates through all rows, skipping the first one
            for (int nRow = 2; nRow <= nEndRow; nRow++)
            {
                var row = worksheet.Cells[
                    nRow, 1, nRow, worksheet.Dimension.End.Column];
                var addressName = row[nRow, 5].GetValue<string>();
                var lat = row[nRow, 6].GetValue<string>();
                var lon = row[nRow, 7].GetValue<string>();
                // skip this country if it already exists in the database
                if (addressesByName.ContainsKey(addressName))
                    continue;
                ...
                addressesByName.Add(addressName, address);
                // increment the counter
                numberOfAddressesAdded++;
            }
            // save all the countries into the Database
            if (numberOfAddressesAdded > 0)
            ...
            var vehicles = _context.Vehicles
                ...
                .ToDictionary(x => (
                    Type: x.Type,
                    State: x.State,
                    VehicleMovement: x.VehicleMovement,
            ...
            for (int nRow = 2; nRow <= nEndRow; nRow++)
            {
                var row = worksheet.Cells[
                    nRow, 1, nRow, worksheet.Dimension.End.Column];
                var type = row[nRow, 1].GetValue<string>();
                var nameAscii = row[nRow, 2].GetValue<string>();
                var state = row[nRow, 3].GetValue<string>();
                var vehicleMovement = row[nRow, 4].GetValue<string>();
                var addressName = row[nRow, 5].GetValue<string>();
                // retrieve country Id by countryName
                var addressId = addressesByName[addressName].Id;
            ...
            {
                Vehicles = numberOfVehiclesAdded,
                Addresses = numberOfAddressesAdded
            });
        }
